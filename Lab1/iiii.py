...
def kaprekar_numbers(digits_number):
    ...
    for var in range(10**(digits_number-1), 10**(digits_number)):
        ...
        if (int('0' + ''.join(lst_numbers[0:len(square)//2 ])) + int('0' + ''.join(lst_numbers[len(square)//2:])) == var or \
            int('0' + ''.join(lst_numbers[0:len(square)//2 +1])) + int('0' + ''.join(lst_numbers[len(square)//2 +1:])) == var or \
            int('0' + ''.join(lst_numbers[0:len(square)//2 -1])) + int('0' + ''.join(lst_numbers[len(square)//2 -1:])) == var) and\
...
def multiply_numbers_not_pal(lst_numbers):
    '''
    list -> list
    Function returns list of nummbers that aren't palindroms and are result of muliplying two different numbers from the list.
    '''
    answer = []
...
#print(multiply_numbers([1,2,11,5]))


def find_two_digits_square(number):
    '''
    int -> list
    Function returns list of tuples of all y and z that given number = y**2 + z**2.
...
def multiply_numbers_ascend(lst_numbers):
    '''
    list -> list
    Function returns list of nummbers which digits are in ascending order and are result of muliplying two different numbers from the list.
    '''
    answer = []
    for var_1 in lst_numbers:
        for var_2 in lst_numbers:
...
def threemorph_numbers(digits_number):
    ...
    for var in range(1, 10**(digits_number)):
        if int(str(var**3)[-len(str(var)):]) == var and len(str(var))== digits_number:
            answer.append(var)
    return answer
...
def multiply_numbers_not_lish(lst_numbers):
    '''
    list -> list
    Function returns list of nummbers that are Lishrel and are result of muliplying two different numbers from the list.
    '''
    answer = []
    for var_1 in lst_numbers:
        for var_2 in lst_numbers:
...
def avtomorph_numbers(digits_number):
    ...
    for var in range(0, 10**(digits_number)):
        if int(str(var**2)[-len(str(var)):]) == var and len(str(var))== digits_number:
            answer.append(var)
    return answer
...
def is_lishrel(number):
    ...
    False
    ''' 
    if not is_Palindrom(int(''.join([el for el in reversed(str(number))])) + number):
        return True
    return False
...
def numbestasks(task = 1):
    '''
    Pre: task = 1 or 2 or 3 or 4
    int -> tuple
    The main function that will perform the chosen variant of the exam and return result.
    '''
    if task == 1:
        main_1()
    if task == 2:
